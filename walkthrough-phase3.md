...
- Added authenticated-client-controlled `POST /v1/messages/{messageId}/ack` and `DELETE /v1/messages/{messageId}` removal endpoints.
...
Observed: the focused Phase 3 integration suite passes 4 tests. The complete repository gates pass with 25 tests across 5 files, including transport, TTL, acknowledgement, explicit deletion, duplicate, isolation, malformed-request, and health checks.
