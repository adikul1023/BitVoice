...
describe('Phase 3 rendezvous service', () => {
  it('moves opaque ciphertext and removes it after acknowledgement', async () => {
    ...
    const ack = await fetch(`${baseUrl}/v1/messages/${message.messageId}/ack`, { method: 'POST' });
    ...
    const deletion = await fetch(`${baseUrl}/v1/messages/${secondMessage.messageId}`, { method: 'DELETE' });
