...
import { acceptResponse, createIdentity, createInvitation, createResponse, getPairingPhrase, listContacts, loadIdentity, parseInvitation, removeContact, saveContact, type Contact, type LocalIdentity, type PairingInvitation } from './identity.js';
import './styles.css';
...
function App() {
  ...
  async function handleImport() {
    try {
      ...
      if (parsed.kind === 'securevoice-pairing-response') {
        ...
        if (!name) return;
        const accepted = await acceptResponse(localIdentity, artifact.trim(), name);
        await refreshContacts();
        setView('contacts');
        setMessage(`Contact saved as unverified. Compare these words: ${accepted.sas}`);
        return;
      }
  ...
  async function markVerified(contact: Contact) { await saveContact({ ...contact, verification: 'verified', verifiedAt: Date.now() }); await refreshContacts(); }
...
function Contacts({ contacts, onVerify, onBlock, onDelete }: { contacts: Contact[]; onVerify: (contact: Contact) => void; onBlock: (contact: Contact) => void; onDelete: (contact: Contact) => void }) { return <><div className="section-heading"><div><p className="kicker">Trusted circle</p><h1>Contacts</h1></div><span className="count">{contacts.length} saved</span></div>{contacts.length === 0 ? <div className="empty"><h2>No contacts yet.</h2><p>Share your verification QR with someone you trust, or import theirs.</p></div> : <div className="contact-list">{contacts.map((contact) => <article className="contact" key={contact.contactId}><div><h2>{contact.displayName}</h2><p className="fingerprint">{contact.contactId}</p><span className={`badge ${contact.verification}`}>{contact.keyChangeState === 'blocked' ? 'Blocked: key changed' : contact.verification}</span></div><div className="actions">{contact.verification === 'unverified' && contact.keyChangeState === 'normal' && <button onClick={() => onVerify(contact)}>Mark verified</button>}{contact.keyChangeState === 'normal' && <button className="quiet" onClick={() => onBlock(contact)}>Block key</button>}<button className="quiet" onClick={() => onDelete(contact)}>Remove</button></div></article>)}</div>}</>; }
...
function Import({ artifact, setArtifact, onImport }: { artifact: string; setArtifact: (value: string) => void; onImport: () => void }) { return <div className="panel"><p className="kicker">Pairing</p><h1>Import an invitation.</h1><p>Paste the signed invitation from the other device. It will be checked before any contact is saved.</p><textarea value={artifact} onChange={(event) => setArtifact(event.target.value)} placeholder="Paste signed invitation JSON" aria-label="Invitation JSON" /><button onClick={onImport}>Review invitation</button></div>; }
