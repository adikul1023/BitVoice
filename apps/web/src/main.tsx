...
function App() {
  ...
  async function markVerified(contact: Contact) { const entered = window.prompt('Enter the six words you compared aloud')?.trim().toLowerCase(); if (!contact.pendingSas || entered !== contact.pendingSas) { setMessage('SAS did not match. The contact remains unverified.'); return; } await saveContact({ ...contact, verification: 'verified', verifiedAt: Date.now(), pendingSas: undefined }); await refreshContacts(); setMessage('Contact verified.'); }
