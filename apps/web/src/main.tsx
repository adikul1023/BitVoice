...
type ScannerConstructor = new (video: HTMLVideoElement, onDecode: (result: ScannerResult) => void, options: { highlightScanRegion: boolean; highlightCodeOutline: boolean }) => Scanner;
...
function App() {
  ...
  async function handleScan(encoded: string) { setView('contacts'); await handleArtifact(encoded); }
  ...
  return <main className="app-shell"><Header /><div className="layout"><aside className="sidebar"><p className="kicker">Device identity</p><strong>{identity.keyId.slice(0, 12)}...</strong><p className="muted">Created {new Date(identity.createdAt).toLocaleDateString()}</p><nav><button className={view === 'contacts' ? 'active' : ''} onClick={() => setView('contacts')}>Contacts</button><button onClick={handleCreateInvitation}>My verification QR</button><button onClick={() => { setScanMode('invitation'); setView('scan'); }}>Scan invitation QR</button><button onClick={() => { setScanMode('response'); setReplacementId(''); setView('scan'); }}>Scan response QR</button></nav></aside><section className="content"><Notice message={message} />{view === 'contacts' && <Contacts contacts={contacts} onVerify={markVerified} onBlock={blockContact} onDelete={deleteContact} />}{view === 'invite' && invitation && <Invite artifact={artifact} title={artifact.includes('pairing-response') ? 'Scan this response.' : 'Scan this invitation.'} />}{view === 'scan' && <Scan mode={scanMode} onScan={handleScan} />}{view === 'review' && pending && <Review pending={pending} onAccept={async () => { setArtifact(pending.response); setView('invite'); }} />}</section></div></main>;
}
...
function Invite({ artifact, title }: { artifact: string; title: string }) { return <div className="panel pairing"><p className="kicker">One-time QR / expires in 10 minutes</p><h1>{title}</h1><QRCodeCanvas value={artifact} size={240} includeMargin /><p className="muted">The other device scans this QR directly. Opening it does not verify a contact; both devices must compare the six-word phrase.</p></div>; }
function Scan({ mode, onScan }: { mode: 'invitation' | 'response'; onScan: (encoded: string) => Promise<void> }) { const [video, setVideo] = useState<HTMLVideoElement | null>(null); const [error, setError] = useState(''); useEffect(() => { if (!video) return; const scanner = new ScannerConstructor(video, (result) => { scanner.stop(); void onScan(result.data); }, { highlightScanRegion: true, highlightCodeOutline: true }); scanner.start().catch((reason: Error) => setError(`Camera unavailable: ${reason.message}`)); return () => scanner.destroy(); }, [video, onScan]); return <div className="panel scanner"><p className="kicker">Camera scan</p><h1>Scan a {mode} QR.</h1><video ref={setVideo} muted playsInline /><p className="muted">Only a signed SecureVoice pairing artifact is accepted. Nothing is saved until its signature and pending invitation are validated.</p>{error && <Notice message={error} />}</div>; }
