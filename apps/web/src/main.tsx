...
function Scan({ mode, onScan, onCancel }: { mode: 'invitation' | 'response'; onScan: (encoded: string) => Promise<void>; onCancel: () => void }) { const [video, setVideo] = useState<HTMLVideoElement | null>(null); const [error, setError] = useState(''); useEffect(() => { if (!video) return; const scanner = new ScannerConstructor(video, (result) => { scanner.stop(); void onScan(result.data); }, { highlightScanRegion: true, highlightCodeOutline: true, returnDetailedScanResult: true }); scanner.start().catch((reason: Error) => setError(`Camera unavailable: ${reason.message}`)); return () => scanner.destroy(); }, [video, onScan]); return <div className="panel scanner"><p className="kicker">Camera scan</p><h1>Scan a {mode} QR.</h1><video ref={setVideo} muted playsInline /><p className="muted">Only a signed SecureVoice pairing artifact is accepted. Nothing is saved until its signature and pending invitation are validated.</p>{error && <Notice message={error} />}<button className="quiet" onClick={onCancel}>Cancel camera</button></div>; }
